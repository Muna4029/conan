...
# Optional build tools that should be skipped (not fail) when not available
OPTIONAL_BUILD_TOOLS = {"cmake", "emcc", "bazel", "qbs", "scons", "premake"}
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        if result is True:
            ...
            # Use skip for optional build tools, fail for required tools
            if tool_name in OPTIONAL_BUILD_TOOLS:
                pytest.skip(f"Optional '{tool_name}' tool version '{version_msg}' is not available")
            else:
                pytest.fail(f"Required '{tool_name}' tool version '{version_msg}' is not available")
        if result is False:
            version_msg = "Any" if tool_version is None else tool_version
