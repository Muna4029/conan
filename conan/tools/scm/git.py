...
class Git:
    ...
    def commit_in_remote(self, commit, remote="origin"):
        """
        Checks that the given commit exists in the remote, with ``ls-remote`` to check
        if the commit appears in the remote's ref space, and falls back to
        ``cat-file -e <commit>^{commit}`` to determine local-only presence.
        ...
        if not remote:
            return False
        # First, check if the commit appears in ls-remote output
        try:
        ...
        # Fall back to cat-file to check if commit exists only locally
        try:
