...
class Git:
    ...
    def commit_in_remote(self, commit, remote="origin"):
        """
        Checks that the given commit exists in the remote, with ``ls-remote`` first,
        then falls back to ``branch -r --contains`` and ``fetch --dry-run`` checks.
        ...
        try:
            ls_remote = self.run(f"ls-remote {remote} {commit}")
        ...
        try:
            ...
            try:
                ...
                # Check if commit exists locally using cat-file to determine if it's a local-only commit
                try:
                    self.run(f"cat-file -e {commit}^{{commit}}")
                    # Commit exists locally but not in remote
                    return False
                except Exception:
                    # Commit doesn't exist locally either
                    return False
